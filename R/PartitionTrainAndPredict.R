...
PartitionTrainAndPredict <- function(study.sample,
                                     ...
                                     model.names = c("ModelGAP", "ModelGerdin",
                                                     "ModelKTS", "ModelRTS"),
                                     n.partitions = 2,
  ...
  if (!IsLength1(outcome.variable.name) | !is.character(outcome.variable.name))
  ...
  if (save.to.results)
    SaveToResults(output.object = return.object, object.name = "predictions.outcome.and.tc")
