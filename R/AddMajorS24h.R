...
AddMajorS24h <- function(study_data) {
  ...
  study_data <- study_data %>%
    mutate(true_major = grepl("446816008|446683008|446115005|178674000|
  ...
  # Paste dos and tos
  study_data <- study_data %>%
    mutate(date_time_surgery = paste(dos, tos)) %>%
    mutate(date_time_ed = paste(doar, toar))
  ...
  study_data <- study_data %>%
    mutate(time_to_surgery = difftime(date_time_surgery, date_time_ed, units = "hours"))

  # Create column for major surgery < 24 h
  study_data <- study_data %>%
    mutate(majors24h = ifelse(true_major == "TRUE" & time_to_surgery <= 24, 1, 0))
