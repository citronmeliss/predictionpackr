...
#' @param outcome Numeric vecotr. The outcome of interest. No default.
...
#' @export
GridsearchBreaks <- function(predictions, outcome,
  ...
  if (!all(levels(as.factor(outcome)) %in% c("0", "1")))
    stop("outcome is not a vector of binary data with only 0 and 1")
  ## Verify that there are no missing values in predictions or outcomes
  if (!all(!is.na(predictions)) | !all(!is.na(outcome)))
  ...
  EstimateLoss <- function(breaks) {
    ...
    loss <- EvaluateWithRocr(predictions = groupings, outcome = outcome,
